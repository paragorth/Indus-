"""Strategy 122: easiest possible planted case: every text is whole Tamil words (no mid-word starts,
no truncation at the end), same number and approximate lengths as Indus texts. Injective key,
window-hit objective (as S114). Does the true key win now?"""
...
vocab=[s for s in (synth.syllabify(w) for w in sorted(lex)) if s and len(s)<=3]; rng0.shuffle(vocab); vocab=vocab[:3000]
...
for segs in base:
    ...
    for seg in segs:
        ...
        while len(out)<len(seg):
            out+=rng0.choices(vocab,wts)[0]
        t.append([code[u] for u in out])   # whole words, may exceed length slightly
    texts.append(t)
N=80
...
allunits=list(C.unit_inventory(lex,'full')); true=[sec[s] for s in top]
def run(seed,iters=40000):
    ...
    return dict(seed=seed,windows=len(W),found=best,true=sum(hit(tuple(true[x] for x in w)) for w in W),correct=sum(a==b for a,b in zip(bk,true)))
